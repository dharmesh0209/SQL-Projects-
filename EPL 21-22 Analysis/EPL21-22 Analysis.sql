...
-- PLAYER ANALYSIS--
 ...
 SELECT Team,Count(*) AS "Squad Size" FROM stats GROUP BY 1 ORDER BY 2 DESC;
 
 -- 2) TOP 5 Goalscorer in the League
 SELECT Player,Position,Apearances,Goals From stats ORDER BY 4 DESC LIMIT 5;
 
-- 3) Top Scorer for Each Team
WITH Goals AS (
SELECT Team,Player,Goals,RANK() OVER(PARTITION BY Team ORDER BY Goals DESC) AS rk
FROM stats)
SELECT Team,Player,Goals FROM gOALS WHERE rk=1;

-- 4) TOP 5 Highest Scoring Midfielders
SELECT Team,Player,Goals FROM stats WHERE Position="Midfielder"
ORDER BY 3 DESC LIMIT 5;

-- 5) Top 5 Highest Scoring Defenders
SELECT Team,Player,Goals FROM stats WHERE Position="Defender"
ORDER BY 3 DESC LIMIT 5;

-- 5) Total Penalties Awareded to each Club
SELECT Team,SUM(Penalties) as Penalties FROM stats GROUP BY 1 ORDER BY 2 DESC;

-- 6) Total Yellow and  Red Cards Received by Club
WITH Card AS(
SELECT Team,player,yellowcards+redcards AS "Cards", 
RANK() OVER(PARTITION BY Team ORDER BY  (yellowcards+redcards) DESC) AS rk FROM stats) 
SELECT Team,Player,Cards FROM Card WHERE rk=1;  

-- 7) Total Red cards by Club
SELECT Team,SUM(RedCards) AS RedCards FROM stats GROUP BY 1 ORDER BY 2 DESC;
